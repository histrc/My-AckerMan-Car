...
uint8_t MPU9250_Init(void)
{
  ...
  HAL_Delay(100); //Delay 100 ms //延时100ms
  ...
  MPU_Set_Rate(200);			//Set the sampling rate to 200Hz //设置采样率50Hz

  MPU_Write_Byte(MPU9250_ADDR,MPU_INT_EN_REG,0X01); ////Turn off all interrupts //关闭所有中断
  MPU_Write_Byte(MPU9250_ADDR,MPU_INTBP_CFG_REG,0x80);//配置中断引脚为低电平触发	
  ...
  if(res==MPU6500_ID) //The device ID is correct, The correct device ID depends on the AD pin //器件ID正确, 器件ID的正确取决于AD引脚
    {
       ...
       MPU_Write_Byte(MPU9250_ADDR,MPU_PWR_MGMT2_REG,0X00); //Acceleration and gyroscope both work //加速度与陀螺仪都工作
		MPU_Set_Rate(200);						       	                //Set the sampling rate to 50Hz //设置采样率为50Hz   
    }
	else return 1;
